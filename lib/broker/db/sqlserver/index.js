...
Database.prototype.deprovisionServiceInstance = function(req, result, callback) {
  ...
  sqlDb.executeSql(db.dbConfig, sql, function(err, data) {
    if (err) {
      ...
      db.log.info(
        'The serivce instance (instanceId=%s) is marked as deprovisioning state.',
        instanceId);
    }
  });
};

Database.prototype.updateServiceInstance = function(instanceId,
  provisioningResult) {
  var db = this;
  ...
  sqlDb.executeSql(db.dbConfig, sql, function(err, data) {
    if (err) {
      ...
      db.log.info(
        'Updated the serivce instance information (instanceId=%s).',
        instanceId);
    }
  });
};
...
Database.prototype.deleteServiceInstance = function(instanceId) {
  ...
  var sql = util.format('DELETE FROM %s where instanceId=\'%s\'', db.instanceTableName,
    instanceId);
  db.log.debug(sql);
  sqlDb.executeSql(db.dbConfig, sql, function(err, data) {
    if (err) {
      ...
      db.log.info(
        'Deleted the serivce instance information (instanceId=%s).',
        instanceId);
    }
  });
};
...
Database.prototype.storeServiceBinding = function(req, result, callback) {
  ...
  sqlDb.executeSql(db.dbConfig, sql, function(err, data) {
    if (err) {
      ...
      db.log.info(
        'The information of the serivce binding (bindingId=%s, instanceId=%s) is created.',
...
Database.prototype.deleteServiceBinding = function(req, result, callback) {
  ...
  sqlDb.executeSql(db.dbConfig, sql, function(err, data) {
    if (err) {
      ...
      db.log.info(
        'The information of the serivce binding (bindingId=%s) is deleted.',
        bindingId);
    }
  });
};
  ...
  sqlDb.executeSql(db.dbConfig, sql, function(err, result) {
    if (err) {
      ...
      if (result.length === 1) {
        ...
        db.log.info(
          'Got the information of the service instance (instanceId=%s)',
          instanceId);
      } else {
        db.log.info(
          'The information of the service instance (instanceId=%s) does not exist.',
          instanceId);
      }
  ...
  sqlDb.executeSql(db.dbConfig, sql, function(err, result) {
    if (err) {
      ...
      if (result.length === 1) {
        ...
        db.log.info(
          'Got the information of the serivce binding (bindingId=%s).',
          bindingId);
      } else {
        db.log.info(
          'The information of the serivce binding (bindingId=%s) is deleted.',
