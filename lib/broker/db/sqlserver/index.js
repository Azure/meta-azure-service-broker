...
Database.prototype.createServiceInstance = function(params, callback) {
  ...
  parameters = encryptionHelper.encryptText(db.encryptKey, params.instance_id, parameters);
...
Database.prototype.updateServiceInstanceProvisioningPendingResult = function(instanceId, provisioningResult, callback) {
  provisioningResult = encryptionHelper.encryptText(this.encryptKey, instanceId, provisioningResult);
...
Database.prototype.getServiceInstance = function(instanceId, callback) {
  ...
  sqlserver.executeSql(db.dbConfig, sql, function(err, result) {
    ...
    var parameters = JSON.parse(encryptionHelper.decryptText(db.encryptKey, instanceId, result[0].parameters));
    var provisioningResult = result[0].provisioningResult;
    if (!(_.isNull(provisioningResult) || _.isUndefined(provisioningResult))) {
      provisioningResult = encryptionHelper.decryptText(db.encryptKey, instanceId, provisioningResult);
    }
...
Database.prototype.createServiceBinding = function(params, result, callback) {
  ...
  parameters = encryptionHelper.encryptText(db.encryptKey, bindingId, parameters);
  bindingResult = encryptionHelper.encryptText(db.encryptKey, bindingId, bindingResult);
...
Database.prototype.getServiceBinding = function(bindingId, callback) {
  ...
  sqlserver.executeSql(db.dbConfig, sql, function(err, result) {
    ...
    var parameters = JSON.parse(encryptionHelper.decryptText(db.encryptKey, bindingId, result[0].parameters));
    var bindingResult = result[0].bindingResult;
    if (!(_.isNull(bindingResult) || _.isUndefined(bindingResult))) {
      bindingResult = encryptionHelper.decryptText(db.encryptKey, bindingId, bindingResult);
    }
