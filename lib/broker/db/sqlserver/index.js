...
var Database = function(opts) {
  ...
  this.log = Logule.init(module, 'sqlserver');
...
Database.prototype.provisionServiceInstance = function(req, result, callback) {
  ...
  var sql = util.format(
    'INSERT INTO %s(instanceId, serviceId, planId, organizationGuid, spaceGuid, parameters, lastOperation, provisioningResult) values (\'%s\', \'%s\', \'%s\', \'%s\', \'%s\', @parameters, \'%s\', \'%s\')',
    db.instanceTableName, instanceId, serviceId, planId, organizationGuid,
    spaceGuid, lastOperation, provisioningResult);
  db.log.debug(sql);

  sqlDb.executeSqlWithParameters(db.dbConfig, sql, parameters, function(err, data) {
    if (err) {
      db.log.error(err);
      callback(err);
    } else {
...
Database.prototype.poll = function(req, lastOperation, result, callback) {
  var db = this;
  var instanceId = req.params.instance_id;
