...
var addListeners = function(broker, operation, params, processResponse, next) {
  ...
  if (broker.listeners(event).length > 0) {
    ...
    var errMsg = util.format(
      '%sing is not implemented for the service %s on this broker.',
      operation,
      serviceId);
  ...
  return function(req, res, next) {
    if (broker.opts.authUser || broker.opts.authPassword) {
      if (req.authorization.basic && req.authorization.basic.username ===
        ...
        var err = 'Invalid auth credentials for requests: ' + req.headers;
...
Handlers.handleProvisionRequest = function(broker, req, res, next) {
  broker.log.info('Processing provision request ' + req.params.instance_id +
    ' from ' +
  ...
  var processResponse = function(err, reply, result) {
    if (err) {
      ...
      broker.db.provision(req, result, next);
    }
  };
  ...
  addListeners(broker, operation, params, processResponse, next);
};
...
Handlers.handlePollRequest = function(broker, req, res, next) {
  ...
  broker.log.info('Processing poll request ' + instanceId + ' from ' +
    req
  ...
  var processResponse = function(err, reply, result) {
    if (err) {
      ...
      if (reply.state == 'succeeded') {
        broker.db.updateInstance(req, result, next);
      }
    }
  };
...
Handlers.handleDeProvisionRequest = function(broker, req, res, next) {
  ...
  broker.log.info('Deprovision request for service instance ' + instanceId +
  ...
  var processResponse = function(err, reply, result) {
    if (err) {
      ...
      broker.db.deprovision(req, result, next);
    }
  };
...
Handlers.handleBindRequest = function(broker, req, res, next) {
  ...
  broker.log.info('Bind request ' + bindingId +
    ' for service instance ' + instanceId + ' from ' + req.connection.remoteAddress +
    ':' +
    req.connection.remotePort);
  ...
  var processResponse = function(err, reply, result) {
    if (err) {
      ...
      broker.db.bind(req, result, next);
    }
  };
...
Handlers.handleUnbindRequest = function(broker, req, res, next) {
  ...
  broker.log.info('Unbind request ' + bindingId +
    ' for service instance ' + instanceId + ' from ' +
    req.connection
  ...
  var processResponse = function(err, reply, result) {
    if (err) {
      ...
      broker.db.unbind(req, result, next);
    }
  };
  ...
  async.series([
      ...
      function(callback) {
        broker.db.getServiceBinding(bindingId, function(err,
          binding) {
          if (err) {
            callback(err);
          } else {
