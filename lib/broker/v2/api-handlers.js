  ...
  if (error) {
    ...
    broker.db.deleteServiceInstance(instanceId, function(err){
      if (err) {
        ...
        broker.log.error('handleProvisionPollResponse - db.deleteServiceInstance: %s', msg);
        return next(new Restify.InternalServerError(msg));
      }
    ...
    broker.db.updateServiceInstanceProvisioningSuccessResult(instanceId, JSON.stringify(result), function(err){
      if (err) {
        ...
        broker.log.error('handleProvisionPollResponse - db.updateServiceInstanceProvisioningSuccessResult: %s', msg);
        return next(new Restify.InternalServerError(msg));
      }
    ...
    broker.db.updateServiceInstanceProvisioningPendingResult(instanceId, JSON.stringify(result), function(err){
      if (err) {
        // CC will continue polling until the broker returns a valid response (200 OK) or the maximum polling duration is reached
        var msg = util.format('Failed in updating the record for (instanceId: %s) in the broker database. DB Error: %j', instanceId, err);
        broker.log.error('handleProvisionPollResponse - db.updateServiceInstanceProvisioningPendingResult: %s', msg);
        return next(new Restify.InternalServerError(msg));
      }
    ...
    if (error.statusCode == HttpStatus.NOT_FOUND) {
      broker.db.deleteServiceInstance(instanceId, function(err){
        if (err) {
          ...
          broker.log.error('handleDeProvisionPollResponse - db.deleteServiceInstance: %s', msg);
          return next(new Restify.InternalServerError(msg));
        }
