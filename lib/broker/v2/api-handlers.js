...
Handlers.authenticate = function(credentials) {
  ...
  return function(req, res, next) {
    if (req.authorization.basic) {
      var userLen = req.authorization.basic.username.length;
      userLen = userLen > credentials.authUser.length ? userLen : credentials.authUser.length;
      const reqAuthUser = Buffer.alloc(userLen);
      reqAuthUser.write(req.authorization.basic.username);
      const credsAuthUser = Buffer.alloc(userLen);
...
Handlers.handleProvisionRequest = function(broker, req, res, next) {
  ...
  if (typeof(accepts_incomplete) == 'undefined' || accepts_incomplete == 'false') {
    ...
    log.error('handleProvisionRequest - %s', msg);
    var body = {
      error: 'AsyncRequired',
      description: msg
    };
...
/**
 * Handles new service update polling requests
 *
 * @this {RestifyServer}
 * @param {Object} error returned by service module
 * @param {Object} reply returned by service module
 ...
 */
  ...
  var handleUpdatePollResponse = function (error, reply, result, broker, req, res, next) {
    var instanceId = req.params.instance_id;
    log.info('handleUpdatePollResponse - (instanceId: %s)', instanceId);

    if (error) {
      log.error('handleUpdatePollResponse: Failed in polling the status for (instanceId: %s). Error: %j', instanceId, error);
    } else {
      log.info('handleUpdatePollResponse: Succeeded in polling the status for (instanceId: %s). State: %j', instanceId, reply);
    }

    res.send(reply.statusCode, reply.value);
    log.info('handleUpdatePollResponse: Sent the update poll response for (instanceId: %s) to cc. State: %j', instanceId, reply);
    return next();
  };
...
Handlers.handlePollRequest = function(broker, req, res, next) {
  ...
  var processResponse = function(error, lastOperation, reply, result) {
    if (lastOperation == 'provision') {
      ...
      return handleUpdatePollResponse(error, reply, result, broker, req, res, next);
    }
...
Handlers.handleDeProvisionRequest = function(broker, req, res, next) {
  ...
  broker.db.getServiceInstance(instanceId, function(err, serviceInstance) {
    ...
    var processResponse = function(error, reply, result) {
      ...
      broker.db.updateServiceInstanceDeprovisioningResult(instanceId, 'deprovision', JSON.stringify(result), function(err){
        if (err) {
          log.error('handleDeProvisionRequest - %j', err);
          res.send(err.statusCode, err);
          return next();
  ...
  async.waterfall([
    ...
    function (serviceInstance, callback) {
      ...
      var params = {
        ...
        instance_id: serviceInstance.instance_id,
        azure: credentialsAndSubscriptionId
      };
