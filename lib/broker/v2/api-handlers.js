...
Handlers.authenticate = function(broker) {
  return function(req, res, next) {
    ...
    var err = util.format('Invalid auth credentials for requests.');
...
Handlers.handleProvisionRequest = function(broker, req, res, next) {
  ...
  var processResponse = function(err, reply, result) {
    if (err) {
      ...
      broker.db.provision(req, result, function(error){
        if (error) {
...
Handlers.handlePollRequest = function(broker, req, res, next) {
  ...
  var processResponse = function(err, lastOperation, reply, result) {
    if (err) {
      if (err.statusCode == HttpStatus.NOT_FOUND) {
        ...
        if (lastOperation == 'deprovision') {
          broker.db.poll(req, lastOperation, result);
        }
  ...
  broker.db.getServiceInstance(instanceId, function(err, serviceInstance) {
    if (err) {
      ...
      res.send(err);
      res.end();
    } else if (_.isEmpty(serviceInstance)) {
      res.status(HttpStatus.GONE);
      res.setHeader('Connection', 'close');
      res.end();
    } else {
...
Handlers.handleDeProvisionRequest = function(broker, req, res, next) {
  ...
  var processResponse = function(err, reply, result) {
    if (err) {
      ...
      broker.db.deprovision(req, result, function(error){
        if (error) {
          res.status(HttpStatus.INTERNAL_SERVER_ERROR);
          res.send(error);
        } else {
