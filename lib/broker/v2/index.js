...
BrokerV2.prototype.start = function () {
  ...
  this.db.createSchema(function (err) {
    ...
    // Fix credentials in the database that have been changed in the Server Pool
    broker.updateBrokerSQLCrededentials(broker.opts.accountPool.sqldb, function(err){
      if (err) {
        log.error('Failed to updade SQL credentials in broker db: %j', err);
      }
    });
...
BrokerV2.prototype.updateBrokerSQLCrededentials = function(sqldbpool, callback) {
  var broker = this;
  var sqldbServiceId = require('../../services/azuresqldb/service.json').id;

  async.waterfall([
    function(callback){
      broker.db.getServiceInstances(callback);
    },
    function (instances, callback) {
      var updateFunctions = [];
      _.each(instances, function (instance) {

        // Only update instances that are of type sqldb and that are part of the sqldbpool
        if (instance['service_id'] === sqldbServiceId && instance['provisioning_result'] && sqldbpool[instance['provisioning_result'].sqlServerName]) {
          var currentPassword = instance['provisioning_result'].administratorLoginPassword;
          var newPassword = sqldbpool[instance['provisioning_result'].sqlServerName].administratorLoginPassword;
          // Only consider update if the password changed
          if (newPassword !== currentPassword) {
            log.info('Should update instance: %s', instance.azureInstanceId);
            updateFunctions.push(async.apply(broker.db.updateSQLPassword.bind(broker.db) , instance, newPassword));
          }
        }
      });
      callback(null, updateFunctions);
    },
    function(updateFunctions, callback) {
