...
BrokerV2.prototype.stop = function() {
  ...
  broker.log.info('Broker has been shutdown.');
};
...
BrokerV2.prototype.start = function() {
  ...
  this.db.createSchema(function(err) {
    if (err) {
      throw new Error('Failed to create schema - ', err.message);
    }
  });

  server.listen(broker.opts.port, function(error) {
    if (error) {
      throw new Error('Broker REST server failed to listen on port %s : %s', broker.opts.port, error);
    }
    broker.log.info('%s broker is listening on port %s', server.name, broker.opts.port);
...
BrokerV2.prototype.createRestServer = function() {
  ...
  server.use(Handlers.authenticate(broker.log, broker.opts.serviceBroker.credentials));
