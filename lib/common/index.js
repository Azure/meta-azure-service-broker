...
module.exports.getConfigurations = function() {
  ...
  var config = {
    ...
    'privilege': {
      'sqldb': {
        'allowToCreateSqlServer': (process.env['AZURE_SQLDB_ALLOW_TO_CREATE_SQL_SERVER'] === 'true')
      }
    },
    'accountPool': {
      'sqldb': {
    ...
    'defaultSettings': {
      'sqldb': {
        'transparentDataEncryption': (process.env['AZURE_SQLDB_ENABLE_TRANSPARENT_DATA_ENCRYPTION'] === undefined || process.env['AZURE_SQLDB_ENABLE_TRANSPARENT_DATA_ENCRYPTION'] === 'true')
      }
    }
  };
...
module.exports.fixNamesIfSpaceScopingEnabled = function (serviceConfig) {
  ...
  if (scopingEnabled) {
    ...
    serviceConfig.plans.forEach(function (plan) {
      plan.id = uuid.v4();
      plan.name = plan.name + ' (' + spaceName + ')'; //brokerConfigurations.serviceBroker.spaceScopingNamePostfix + ')';
    });
  }
