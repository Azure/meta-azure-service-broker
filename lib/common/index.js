...
function getLogger(moduleName) {
  ...
  var moduleWinstonConfig = winstonConfig;
  moduleWinstonConfig['console']['label'] = moduleName;
  winston.loggers.add(moduleName, moduleWinstonConfig);
  return winston.loggers.get(moduleName);
}
...
var addLogger = function (moduleName, label) {
  var moduleWinstonConfig = winstonConfig;
  moduleWinstonConfig['console']['label'] = label;
  winston.loggers.add(moduleName, moduleWinstonConfig);
};
module.exports.addLogger = addLogger;
    
addLogger(LOG_CONSTANTS.BROKER, 'Meta Azure Service Broker');

addLogger(LOG_CONSTANTS.BROKER_DB, 'broker database');

addLogger(LOG_CONSTANTS.COMMON, 'common');

var log = require('winston').loggers.get(LOG_CONSTANTS.COMMON);
// end
...
var mergeCommonHeaers = module.exports.mergeCommonHeaders = function(message, headers) {
...
module.exports.generateCustomHeaders = function(message) {
  var options = {
    customHeaders: mergeCommonHeaers(message, {})
  };
  return options;
};
