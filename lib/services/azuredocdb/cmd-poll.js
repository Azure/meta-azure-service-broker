  ...
  this.poll = function(docDb, callback) {
    ...
    docDb.getDocDbAccount(resourceGroupName, docDbAccountName, function(err, account) {
      ...
      if (lastoperation === 'provision') {
        ...
        log.info('Getting the provisioning state of the docDb account %s: %j', lastoperation, docDbAccountName, accountState);
      
        if (accountState == 'Succeeded') {
          var createDbError = null;
          ...
          async.waterfall([
            ...
            function(masterKey, callback) {
              docDb.createDocDbDatabase(documentEndpoint, masterKey, docDbName, function(err, database) {
                ...
                */
                callback(err, masterKey, database);
              });
            }
          ], function(err, masterKey, database) {
            if (err) {
              callback(err);
            } else {
              reply.state = 'succeeded';
              reply.description = 'Created the docDb';
              _.extend(provisioningResult, {documentEndpoint: documentEndpoint, masterKey: masterKey, database: database});
        ...
        if (!err) {
          var accountState = account.properties.provisioningState;
          log.info('Getting the deprovisioning state of the docDb account %s: %j', lastoperation, docDbAccountName, accountState);
