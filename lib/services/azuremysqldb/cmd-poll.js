...
var mysqldbPoll = function (params) {
    ...
    log.info(util.format('cmd-poll: resourceGroupName: %s, mysqlServerName: %s', resourceGroupName, mysqlServerName));
    ...
    this.poll = function (mysqldbOperations, next) {
            ...
            mysqldbOperations.getServer(function (err, result) {
                if (err) {
                    ...
                    if (firewallRules !== null) {
                        ...
                        reply.description = 'Created server ' + mysqlServerName + '. Creating firewall rules.';
                    } else {
                        reply.state = 'succeeded';
                        reply.description = 'Created server ' + mysqlServerName + '.';
                    }
                    callback(null);
                }
            });
        ...
        function validateFirewallRules(callback) {
            ...
            async.whilst(
                ...
                function(cb) {
                    ...
                    mysqldbOperations.checkComplete(ruleStatusUrls[i], function (err, status) {
                        if (err) {
        ...
        if (lastoperation === 'provision') {
            ...
            if (serverPollingUrl) { // server is created if not exist, to check firewall rule status
                ...
                mysqldbOperations.checkComplete(serverPollingUrl, function(err, status){
                    if (err) {
                        return next(err);
                    }
                    ...
                    if (status === 'InProgress') {
                        reply.state = 'in progress';
                        reply.description = 'Creating server ' + mysqlServerName + '.';
                        ...
                        reply.state = 'failed';
                        reply.description = 'Failed to create server ' + mysqlServerName + '.';
                        next(null, pollingResult);
                    }
                });
            } else {
                validateFirewallRules(function(err){
                    ...
                    if (provisioningResult.ruleStatusUrls.length === 0) {
                        reply.state = 'succeeded';
                        reply.description = 'Created server ' + mysqlServerName + '. Created firewall rules.';
                    } else {
                        reply.state = 'in progress';
                        reply.description = 'Created server ' + mysqlServerName + '. Creating firewall rules.';
                    }
