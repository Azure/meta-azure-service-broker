...
var cacheProvision = function(params) {
  ...
  Config.plans.forEach(function (item) {
    if (planId === item.id) {
      params.parameters.parameters.sku = {
  ...
  this.provision = function(redis, next) {
    ...
    async.waterfall([
      ...
      function(callback) {
        ...
        log.debug('Redis Cache, redis.provision, reqParams: %j', reqParams);
        redis.provision(resourceGroupName, cacheName, reqParams, function(err, result) {
          if (err) {
