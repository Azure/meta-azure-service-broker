...
Handlers.provision = function(log, params, next) {
  ...
  var reqParams_name = ['resource_group_name', 'namespace_name', 'location', 'type', 'messaging_tier'];
  var len = reqParams_name.length;
  for (var i = 0; i<len; i++) {
    if (!_.has(reqParams, reqParams_name[i])) {
      var err = new Error(reqParams_name[i] + ' in configuration needed.');
  ...
  var config = {
    resourceGroupName : reqParams[reqParams_name[0]],
    namespaceName : reqParams[reqParams_name[1]],
    location : reqParams[reqParams_name[2]],
    sbType : reqParams[reqParams_name[3]],
    sbTier : reqParams[reqParams_name[4]]
  };
