...
sqldbOperations.prototype.executeSql = function (config, sql, isDeleteFirewallRuleAfter, callback) {
    ...
    var conn = sqlDb.connect(config, function(err) {
        if (err) {
            ...
            } else if (!err.message.startsWith('Login failed for user')) {
                // It is for the scenario creating a new server, the master database is not ready at the same time the database created by the broker is ready.
                return setTimeout(function(){that.executeSql(config, sql, isDeleteFirewallRuleAfter, callback);}, 1000);
            }
