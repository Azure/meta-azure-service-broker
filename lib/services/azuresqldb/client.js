...
sqldbOperations.prototype.setTransparentDataEncryption = function (callback){
    ...
    var headers = common.mergeCommonHeaders(that.log, 'sqldb client - setTransparentDataEncryption', that.standardHeaders);
    that.PUT(that.transparentDataEncryptionUrl, headers, data, API_VERSIONS.SQL, function(err, res, body) {

        common.logHttpResponse(that.log, res, 'Set transparent data encryption', true);

        if (err) {
            that.log.error('sqldb client: setTransparentDataEncryption: err: %j', err);
            callback(err, res);
        } else {
            that.log.info('sqldb client: setTransparentDataEncryption: body: %j', body);
            res.body = body;
            callback(null, res);
        }
    });
