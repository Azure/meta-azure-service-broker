...
sqldbOperations.prototype.setParameters = function (accessToken, resourceGroupName, sqlServerName, sqldbName) {
    ...
    this.id = 'subscriptions/{0}/resourceGroups/{1}/providers/Microsoft.Sql/servers/{2}/databases/{3}'
        .format(this.azure.subscription_id, resourceGroupName, sqlServerName, sqldbName);
...
sqldbOperations.prototype.deleteDatabase = function (callback) {
    ...
    that.DELETE(that.sqldbUrl, that.standardHeaders, API_VERSION_FOR_SQL, function (err, res) {
        ...
        that.log.debug('sqldb client: deleteDatabase');

        if (err) {
            that.log.debug('sqldb client: deleteDatabase: err %j', err);
            callback(err, null);
        } else {
            that.log.debug('sqldb client: deleteDatabase');
