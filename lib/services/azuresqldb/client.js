...
sqldbOperations.prototype.setParameters = function (resourceGroupName, sqlServerName, sqldbName) {
    ...
    this.sqldbUrl = util.format('%s/databases/%s', this.serverUrl, sqldbName);

    this.firewallRuleUrl = this.serverUrl + '/firewallRules/';
    this.transparentDataEncryptionUrl = this.sqldbUrl + '/transparentDataEncryption/current';
    this.blobAuditingPolicyUrl = this.sqldbUrl + '/auditingSettings/Default';

    this.sqlServerName = sqlServerName;
    this.sqldbName = sqldbName;
    this.resourceType = 'Microsoft.Sql/servers/databases';

    this.id = this.sqldbUrl;
...
sqldbOperations.prototype.setBlobAuditingPolicy = function (body, callback) {
    var that = this;

    var headers = common.mergeCommonHeaders(that.log, 'sqldb client - setBlobAuditingPolicy', that.standardHeaders);
    that.PUT(that.blobAuditingPolicyUrl, headers, body, API_VERSIONS.SQL, function(err, res, body) {
        common.logHttpResponse(that.log, res, 'SetBlobAuditingPolicy', true);
        res.body = body;
        callback(err, res);
    });
};
