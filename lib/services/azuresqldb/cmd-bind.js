    ...
    this.bind = function (sqldbOperations, next) {
        ...
        async.waterfall([
            ...
            function (callback) {   // get status of server
                log.info('sqldb cmd-bind: async.waterfall/check existence of server');
                sqldbOperations.getServer({}, function (err, result) {
                    if (err) {
                        log.error('sqldb cmd-bind: async.waterfall/check existence of sql server: err: %j', err);
                        callback(err);
                    } else {
                        log.info('sqldb cmd-bind: async.waterfall/check existence of sql server: result: %j', result);
                        callback(null, result);
                    }
                });
            }
        ], function (err, result) {
            log.info('sqldb cmd-bind: async.waterfall/final callback: result: ', result);
            next(err, result);
        });
    };
};
