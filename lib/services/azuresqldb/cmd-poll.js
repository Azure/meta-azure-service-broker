...
var sqldbPoll = function (params) {
    ...
    this.poll = function (sqldbOperations, next) {
        ...
        async.waterfall([
            ...
            function(result, callback) {   // Set sqldBlobAuditingPolicy
                if (sqldBlobAuditingPolicy){
                    log.info('sqldb cmd-poll: Set sqldBlobAuditingPolicy');
                    sqldbOperations.setBlobAuditingPolicy(sqldBlobAuditingPolicy, function (err, auditingResult) {
                        if (err) {
                            callback(err);
                        } else if (auditingResult.statusCode === HttpStatus.OK ) {
                            log.info('sqldb cmd-poll: sqldBlobAuditingPolicy :success');
                            callback(null, result);
                        } else {
                            log.error('sqldb cmd-poll: sqldBlobAuditingPolicy, unexpected error: auditingResult: %j', auditingResult);
                            callback(Error(auditingResult.body.message));
                        }
                    });
                } else {
                    callback(null, result);
