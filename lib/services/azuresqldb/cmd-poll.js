        ...
        async.waterfall([
            ...
            function(result, callback) {   // Set Transparent data encryption  
                if ( lastoperation === 'provision' && result.value.state === 'succeeded' && tde === true){
                    log.info('sqldb cmd-poll: Set transparent data encryption: Enabled');
                    sqldbOperations.setTransparentDataEncryption(function (err, tdeResult) {
                        if (err) {
                            ...
                            log.info('sqldb cmd-poll: setTransparentDataEncryption: success');
                            callback(null, result);
                        } else {
                            log.error('sqldb cmd-poll: setTransparentDataEncryption, unexpected error: tdeResult: %j', tdeResult);
