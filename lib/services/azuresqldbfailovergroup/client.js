...
sqldbfgOperations.prototype.executeSql = function (config, sql, callback) {
  ...
  var conn = sqlDb.connect(config, function(err) {
    ...
    log.info('client: it is going to execute sql: %s', sql);
    ...
    req.query(sql, function(err) {
      ...
      log.info('client: executed sql: %s', sql);
      callback(null);
    });
  });
};
...
sqldbfgOperations.prototype.createFailoverGroup = function (callback) {
  var that = this;
