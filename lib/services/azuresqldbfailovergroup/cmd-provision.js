...
var sqldbfgProvision = function (params) {
  ...
  var readWriteEndpoint = reqParams.readWriteEndpoint || {};
    ...
    async.waterfall([
      ...
      function (failoverGroupResult, callback) {
        if (params.plan_id === '5a75ffc1-555d-4193-b60b-eb464069f913') { // register an existing failover group
          ...
          sqldbfgOperations.createFailoverGroup(readWriteEndpoint, function (err, fgPollingUrl) {
            if (err) {
  ...
  this.areUserRolesValid = function () {
    return common.isValidStringArray(userRoles);
  };

  this.areUserPermissionsValid = function () {
    return common.isValidStringArray(userPermissions);
  };
  ...
  this.parametersNotProvided = function () {
    ...
    if (!_.isObject(readWriteEndpoint) || !_.isString(readWriteEndpoint.failoverPolicy)) {
      ret.push('readWriteEndpoint');
    }
    return ret;
  };
