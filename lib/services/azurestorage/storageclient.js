...
function checkNameAvailability(storageAccountName, callback) {
  msRestRequest.POST(
    ...
    common.mergeCommonHeaders(log, 'Storage - checkNameAvailability', {}),
...
function createStorageAccount(resourceGroupName, storageAccountName, accountParameters, callback) {
  msRestRequest.PUT(
    util.format(
      ...
      resourceGroupName,
      storageAccountName),
    common.mergeCommonHeaders(log, 'Storage - createStorageAccount', {}),
    accountParameters,
    API_VERSIONS.STORAGE_ACCOUNT,
    callback
  );
...
exports.provision = function(resourceGroupName, groupParameters, storageAccountName, accountParameters, next) {
  async.series([
      ...
      function(callback) {
        resourceGroup.createOrUpdate('Storage', azureProperties, resourceGroupName, groupParameters, log,
          function(err, response, body) {
            callback(err, {
...
function getProperties(resourceGroupName, storageAccountName, callback) {
  msRestRequest.GET(
    util.format(
      ...
      resourceGroupName,
      storageAccountName),
    common.mergeCommonHeaders(log, 'Storage - getProperties', {}),
    API_VERSIONS.STORAGE_ACCOUNT,
    callback
  );
}
...
function deleteMethod(resourceGroupName, storageAccountName, callback) {
  msRestRequest.DELETE(
    util.format(
      ...
      resourceGroupName,
      storageAccountName),
    common.mergeCommonHeaders(log, 'Storage - deleteMethod', {}),
    API_VERSIONS.STORAGE_ACCOUNT,
    callback
  );
}
...
function listKeys(resourceGroupName, storageAccountName, callback) {
  msRestRequest.POST(
    util.format(
      ...
      resourceGroupName,
      storageAccountName),
    common.mergeCommonHeaders(log, 'Storage - listKeys', {}),
    null,
    API_VERSIONS.STORAGE_ACCOUNT,
    callback
  );
