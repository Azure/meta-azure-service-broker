...
Handlers.provision = function(log, params, next) {
  ...
  if (_.has(reqParams, 'resource_group_name')) {
  ...
  if (_.has(reqParams, 'storage_account_name')) {
    storageAccountName = reqParams.storage_account_name;
  } else {
    storageAccountName = STORAGE_ACCOUNT_NAME_PREFIX + instanceId.replace(
      /-/g, '').slice(0, 22);
  }
  ...
  var accountType;
  if (_.has(reqParams, 'account_type')) {
