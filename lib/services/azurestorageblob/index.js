...
Handlers.provision = function(log, params, next) {
  ...
  var resourceGroupName = reqParams.resource_group_name ||
    RESOURCE_GROUP_NAME_PREFIX + instanceId;
  var groupParameters = {
    location: LOCATION
  };

  var storageAccountName = reqParams.storage_account_name ||
    STORAGE_ACCOUNT_NAME_PREFIX + instanceId
...
Handlers.poll = function(log, params, next) {
  ...
  storageBlobClient.poll(resourceGroupName, storageAccountName,
    function(err, state) {
      var reply = {
        state: '',
        description: '',
      ...
      if (lastOperation == 'provision') {
        if (!err) {
          log.info(
            'Getting the provisioning state of the storage account %s: %j',
            storageAccountName, state);
...
Handlers.deprovision = function(log, params, next) {
  ...
  storageBlobClient.deprovision(resourceGroupName, storageAccountName,
    function(err) {
      if (err) {
        var error = {
          statusCode: err.statusCode,
...
Handlers.unbind = function(log, params, next) {
  ...
  storageBlobClient.unbind(resourceGroupName, storageAccountName,
    function(err) {
      if (err) {
        var error = {
          statusCode: err.statusCode,
